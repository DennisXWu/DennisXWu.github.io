...
2、配置类，用来启动Spring容器以及扫描包路径下的Bean
...
![]({{ site.url }}/assets/img/spring/16.1.png)


## 2、通过Java装配Bean

   当需要将第三方库中的组件装配到你的应用中，在这种情况下无法添加@Component和@Autowired注解，因此需要通过显示装配Bean，包括两种：**JAVA和XML**。由于通过XML去装配过于复杂且很少使用，暂且不考虑。

 1、通过@Bean注解一个方法来返回一个对象
...
2、通过Spring容器可以获取到这个bean对象

```java
   @Test
    public void testJavaBean() {
        Audience audience = (Audience) applicationContext.getBean("getAudience");
        audience.say();
    }
```
